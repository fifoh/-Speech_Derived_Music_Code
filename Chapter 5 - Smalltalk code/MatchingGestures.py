...
plt.plot(series, label='Series', color='black')

# Offset the query to the left
query_x_range = range(query_offset, query_offset + len(query))
plt.plot(query_x_range, query, label='Query', color='green')

# Add a larger box around the query
query_min, query_max = min(query), max(query)  # Get min/max for y-range
query_box = patches.Rectangle(
    (query_offset - margin_x / 2, query_min - margin_y / 2),  # Bottom-left corner (shifted for margin)
    len(query) + margin_x,  # Width (increased)
    (query_max - query_min) + margin_y,  # Height (increased)
    linewidth=3,  # Slightly thicker border
    edgecolor='green',
    facecolor='none'
)
plt.gca().add_patch(query_box)  # Add to the plot
...
for i, match in enumerate(matches_list):
    ...
    if i == 0:
        color, alpha = 'green', 0.7  # Best match: solid green
    else:
        color, alpha = 'green', max(0.3, 0.7 - (i * 0.1))  # Others: fading green

    plt.axvspan(start, end, color=color, alpha=alpha)

plt.title(f"Top {top_n} Matches")
plt.legend()
plt.show()
